...
export default async function InterviewPrep() {
  ...
  return (
    <Card className="mr-4 flex flex-col items-center text-center">
      <div className="p-2">
        <Heading color="amber">
          Welcome to my Interview Preparation Guide.
        </Heading>
        <div className="mt-3 mb-3">
          <Text color="amber" highContrast>
            Are you currently preparing or thinking about preparing for a
            technical interview?{" "}
            <Blockquote color="blue">You are in the right spot.</Blockquote>
            <br />I will organise everything you need to be ready for that
            interview in one spot down below. Just grab your coffee and start
            today.
          </Text>
        </div>
        <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {posts.filter(Boolean).map((post) => (
            <li key={post.title}>
              <Card>
                <strong>
                  <Link color="brown" href={`/interview-prep/${post.slug}`}>
                    {post.title}
                  </Link>
                </strong>
                <Text as="p">
